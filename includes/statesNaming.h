#include "Arduino.h"
	...
	#define OPERATING_FREQUENCY 850
#else
	#define OPERATING_FREQUENCY 433
#endif

/* [수정필요]  BROADCAST_ADDRESS*/
...
#endif
  E310_SUCCESS = 1,
  ERR_E310_UNKNOWN,	/* something shouldn't happened */
  ERR_E310_NOT_SUPPORT,
  ERR_E310_NOT_IMPLEMENT,
  ERR_E310_NOT_INITIAL,
  ERR_E310_INVALID_PARAM,
  ERR_E310_DATA_SIZE_NOT_MATCH,
  ERR_E310_BUF_TOO_SMALL,
  ERR_E310_TIMEOUT,
  ERR_E310_HARDWARE,
  ERR_E310_HEAD_NOT_RECOGNIZED,
  ERR_E310_NO_RESPONSE_FROM_DEVICE,
  ERR_E310_WRONG_UART_CONFIG,
  ERR_E310_WRONG_FORMAT,
  ERR_E310_PACKET_TOO_BIG
} Status;
...
static String getResponseDescriptionByParams(byte status){
	switch (status)
	{
	  case E310_SUCCESS:
		return F("Success");
		break;
	  case ERR_E310_UNKNOWN:
		return F("Unknown");
		break;
	  case ERR_E310_NOT_SUPPORT:
		return F("Not support!");
		break;
	  case ERR_E310_NOT_IMPLEMENT:
		return F("Not implement");
		break;
	  case ERR_E310_NOT_INITIAL:
		return F("Not initial!");
		break;
	  case ERR_E310_INVALID_PARAM:
		return F("Invalid param!");
		break;
	  case ERR_E310_DATA_SIZE_NOT_MATCH:
		return F("Data size not match!");
		break;
	  case ERR_E310_BUF_TOO_SMALL:
		return F("Buff too small!");
		break;
	  case ERR_E310_TIMEOUT:
		return F("Timeout!!");
		break;
	  case ERR_E310_HARDWARE:
		return F("Hardware error!");
		break;
	  case ERR_E310_HEAD_NOT_RECOGNIZED:
		return F("Save mode returned not recognized!");
		break;
	  case ERR_E310_NO_RESPONSE_FROM_DEVICE:
		return F("No response from device! (Check wiring)");
		break;
	  case ERR_E310_WRONG_UART_CONFIG:
		return F("Wrong UART configuration! (BPS must be 9600 for configuration)");
		break;
	  case ERR_E310_PACKET_TOO_BIG:
